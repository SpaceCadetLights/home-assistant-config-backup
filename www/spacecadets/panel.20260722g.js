...
class SpaceCadetsPanel extends HTMLElement {
  ...
  _openPlaylistModal() {
    ...
    this._playlistPollN = 0; this._loadPlaylistPicker();
  }
  ...
  _renderPlaylistModal(full) {
    ...
    const grid = filtered
      .map((p) => {
        ...
        return `<button type="button" class="sc-pl-item${on}${p.can_add === false ? " readonly" : ""}" data-act="pl-pick" data-uri="${this._esc(p.uri || "")}" data-name="${this._esc(p.name || "")}" title="${p.contains ? "Already on this playlist" : ""}" ${p.can_add === false ? "disabled" : ""}>
    ...
    if (full) {
      ...
      m.querySelector("#sc-pl-bd").addEventListener("click", () => this._closePlaylistModal());
      m.querySelector("#sc-pl-x").addEventListener("click", () => this._closePlaylistModal());
      ...
      m.querySelectorAll("[data-act='pl-pick']").forEach((el) => {
        el.addEventListener("click", () => this._addTrackToPlaylist(el.dataset.uri, el.dataset.name));
      });
      requestAnimationFrame(() => requestAnimationFrame(() => m.classList.add("open")));
      return;
    }
    ...
    if (loading && items.length) {
      if (!scan) {
        ...
        m.querySelector(".sc-pl-modal")?.appendChild(scan);
      }
  ...
  _showPlaylistConfirm({ title, detail, ok }) {
    const m = this.querySelector("#sc-playlist-modal");
    if (!m) return;
    const modal = m.querySelector(".sc-pl-modal");
    if (!modal) return;
    // Freeze the list — replace body with a clear confirmation flash.
    clearTimeout(this._playlistPollT);
    this._playlistMembershipLoading = false;
    const body = `
      <div class="sc-pl-confirm ${ok ? "ok" : "err"}" role="status" aria-live="polite">
        <div class="sc-pl-confirm-mark">${ok ? "✓" : "!"}</div>
        <div class="sc-pl-confirm-title">${this._esc(title)}</div>
        ${detail ? `<div class="sc-pl-confirm-detail">${this._esc(detail)}</div>` : ""}
      </div>`;
    // Keep header; swap search + grid for confirmation.
    const search = modal.querySelector(".sc-pl-search");
    const grid = modal.querySelector("#sc-pl-grid") || modal.querySelector(".sc-pl-loading") || modal.querySelector(".sc-pl-empty");
    const scan = modal.querySelector(".sc-pl-scan");
    if (scan) scan.remove();
    if (search) search.remove();
    if (grid) {
      const holder = document.createElement("div");
      holder.innerHTML = body.trim();
      grid.replaceWith(holder.firstElementChild);
    } else {
      modal.insertAdjacentHTML("beforeend", body);
    }
  }

  async _addTrackToPlaylist(uri, name) {
    if (!uri || this._playlistSaving) return;
    this._playlistSaving = true;
    clearTimeout(this._playlistPollT);
    const btn = this.querySelector(`[data-uri="${CSS.escape(uri)}"]`);
    if (btn) btn.classList.add("busy");
    // Immediate tap feedback so the click feels intentional.
    this._showPlaylistConfirm({
      title: `Saving to ${name || "playlist"}…`,
      detail: "",
      ok: true,
    });
    let resp = null;
    ...
    const ok = status === "added" || status === "already";
    const plName = (resp && resp.playlist && resp.playlist.name) || name || "playlist";
    const trackName = (resp && resp.track && resp.track.name) || (this._playlistTrack && this._playlistTrack.name) || "Song";

    if (ok) {
      const title = status === "added" ? "Saved to playlist" : "Already on playlist";
      const detail = status === "added"
        ? `${trackName} → ${plName}`
        : `${trackName} is already on ${plName}`;
      this._showPlaylistConfirm({ title, detail, ok: true });
      const edited = Number((resp.playlist && resp.playlist.last_edited) || Date.now() / 1000);
      this._playlistItems = (this._playlistItems || []).map((p) =>
        p.uri === uri
          ? { ...p, contains: true, last_edited: edited }
          : p
      );
      // Close quickly so they land back on the music screen with certainty.
      setTimeout(() => this._closePlaylistModal(), status === "already" ? 900 : 1100);
      return;
    }

    this._showPlaylistConfirm({
      title: "Couldn't save",
      detail: (resp && resp.message) || `Could not add to ${plName}. Try again.`,
      ok: false,
    });
    setTimeout(() => {
      if (!this._playlistOpen) return;
      // Restore the picker so they can try another playlist.
      this._renderPlaylistModal(true);
      this._renderPlaylistModal(false);
    }, 1400);
  }
