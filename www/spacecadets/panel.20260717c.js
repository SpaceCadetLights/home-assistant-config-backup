...
class SpaceCadetsPanel extends HTMLElement {
  ...
  async _renderCrewTrail(animateIn = false) {
    ...
    m.innerHTML = `
      <div class="sc-modal-backdrop" id="sc-modal-bd"></div>
      <div class="sc-modal glass sc-modal-trail">
        <div class="sc-modal-head">
  ...
  async _loadCrewTrailDay({ reuseMap = true } = {}) {
    ...
    try {
      ...
      if (!canReuse) {
        ...
        this._trailTiles = L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", {
          maxZoom: 20,
          minZoom: 2,
          subdomains: "abcd",
...
.sc-trail-map-wrap {
  ...
  height: min(52vh, 460px); min-height: 280px; background: #0b1220;
  z-index: 2;
  flex: 1 1 auto;
  /* Allow Leaflet to own gestures — do NOT use touch-action:none on ancestors */
...
.sc-trail-map img.leaflet-tile {
  /* HA / card-mod sometimes constrains images */
