...
class SpaceCadetsPanel extends HTMLElement {
  ...
  _openPlaylistModal() {
    ...
    this._loadPlaylistPicker(false);
  }
  ...
  _renderPlaylistModal(full) {
    ...
    const grid = filtered
      .map((p) => {
        ...
        const dot = p.contains ? `<span class="sc-pl-dot" title="Already on playlist"></span>` : "";
        return `<button type="button" class="sc-pl-item${on}${p.can_add === false ? " readonly" : ""}" data-act="pl-pick" data-uri="${this._esc(p.uri || "")}" data-name="${this._esc(p.name || "")}" ${p.can_add === false ? "disabled" : ""}>
          <span class="sc-pl-thumb ${art ? "" : "noart"}" style="${art}">${art ? "" : "♫"}${dot}</span>
    ...
    if (full) {
      ...
      <div class="sc-modal sc-pl-modal glass">
        ...
        ${body}
        ${loading && items.length ? `<div class="sc-pl-scan">Checking which playlists already have this track…</div>` : ""}
      </div>`;
  ...
  async _loadPlaylistPicker(withMembership) {
    this._playlistLoading = !withMembership;
    this._playlistMembershipLoading = !!withMembership;
    if (!withMembership) this._renderPlaylistModal(false);
    let resp = null;
    try {
      resp = await this._callResp(
        "launchpad_music",
        "playlist_picker",
        { entity_id: this._studioPlayer, check_membership: !!withMembership },
        {}
      );
    } catch (e) {
      resp = null;
      this._playlistError = String(e?.message || e || "Failed to load playlists");
    }
    if (!this._playlistOpen) return;
    if (!resp) {
      this._playlistLoading = false;
    ...
    this._playlistMembershipLoading = false;
    this._renderPlaylistModal(false);
    if (!withMembership) this._loadPlaylistPicker(true);
  }
...
.sc-pl-dot {
  ...
  background: #34d399; border: 2px solid #0b0318;
