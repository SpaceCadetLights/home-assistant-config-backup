...
class SpaceCadetsPanel extends HTMLElement {
  constructor() {
    ...
    this._playlistMembershipLoading = false;
    this._playlistItems = [];
    this._playlistTrack = null;
    this._musicMode = this._loadMusicMode(); // "native" | "assistant"
    this._musicSearchOpen = false;
    this._musicSearchScope = "all"; // all | artist | playlist | genre
    this._musicSearchQuery = "";
    this._musicSearchResults = [];
    this._musicSearchLoading = false;
    this._musicSearchError = null;
    this._massEntryId = "01K50H4V9P53VZ36HSWHBZ13WP";
  }
  ...
  async _callResp(domain, service, data = {}, target = {}) {
    if (!this._hass) return null;
    try {
      // HA callService args: (domain, service, data, target, notifyOnError, returnResponse).
      // Prefer callWS so return_response is explicit — SupportsResponse.ONLY services need it.
      const msg = {
        type: "call_service",
        domain,
        service,
      ...
      if (target && Object.keys(target).length) msg.target = target;
  ...
  _mediaInfo(id) {
    ...
    let volPct = vol != null ? Math.round(vol * 100) : null;
    if (this._volDragging && this._volDragEntity === id && this._volDragValue != null) {
      volPct = Math.round(Number(this._volDragValue));
    }
    return {
      id,
      name: this._name(id),
  ...
  _transportHtml(info) {
    return `
            ...
            <button class="sc-pl-save" data-act="open-playlist" title="Save to playlist" aria-label="Save to playlist">${this._ic("save")}</button>
            <button class="sc-ms-btn" data-act="open-music-search" title="Search music" aria-label="Search music">${this._ic("search")}</button>`;
  }
  ...
  _closeAssist() {
    ...
    this._assistPendingContinue = false;
    ...
    if (this._assistAudioEl) { try { this._assistAudioEl.pause(); } catch (_) {} }
    this._lockAppScroll(false);
    m.classList.remove("open");
  ...
  async _assistStartVoice() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      // In the HA companion app the webview often loads over http (not a secure
      // context, so the web mic API is missing). Hand off to the app's NATIVE
      // assist, which records with native audio. Text chat still works here.
      if (this._assistNativeVoice()) {
        this._assistNote("Opening the app's voice assistant… For voice inside this chat, open Space Cadets over your HTTPS (remote) URL — or just type here.");
        this._assistSetState("idle");
        return;
      }
      this._assistNote(window.isSecureContext
        ? "This browser has no microphone API available — you can type instead."
        : "Voice needs a secure (HTTPS) connection — open Space Cadets via your remote (Nabu Casa / Homeway) URL. You can type here meanwhile.");
      this._assistSetState("idle");
      return;
    }
    this._assistMicStarting = true;
    this._assistPendingContinue = false;
    // Create + resume the AudioContext inside the user gesture (iOS/WKWebView requirement).
    const AC = window.AudioContext || window.webkitAudioContext;
    let ctx = null;
    try { ctx = new AC({ sampleRate: 16000 }); } catch (_) { try { ctx = new AC(); } catch (_) {} }
    ...
    try {
      ...
      const n = (e && e.name) || "";
      if (n === "NotAllowedError" || n === "SecurityError") this._assistNote("Microphone permission is blocked — allow mic access for the Home Assistant app in system Settings, then tap the mic to retry.");
      ...
      else this._assistNote("Microphone unavailable (" + (n || "error") + ") — you can type instead.");
      this._assistSetState("idle");
      return;
    }
    ...
    try {
      ...
      this._assistLastInputVoice = true;
  ...
  _assistToggleMic() {
    if (this._assistState === "listening") { this._assistPendingContinue = false; this._assistStopVoice(true); }
    else { this._assistStartVoice(); }
  }

  _assistMaybeContinue() {
    if (!this._assistPendingContinue) return;
    this._assistPendingContinue = false;
    if (!this._assistOpen) return;
    if (this._assistState === "listening" || this._assistMicStarting) return;
    // Small gap so the tail of the spoken reply doesn't get captured.
    setTimeout(() => {
      if (this._assistOpen && this._assistState !== "listening" && !this._assistMicStarting) this._assistStartVoice();
    }, 350);
  }

  _assistNativeVoice() {
    // Trigger the companion app's native Assist (records with native audio, so it
    // works even over http). Returns true if the handoff was fired.
    try {
      const ext = this._hass && this._hass.auth && this._hass.auth.external;
      if (!ext || !ext.fireMessage) return false;
      const payload = { start_listening: true };
      if (this._assistPipelineId) payload.pipeline_id = this._assistPipelineId;
      ext.fireMessage({ type: "assist/show", payload });
      return true;
  ...
  _assistPipelineEvent(ev) {
    ...
    if (t === "run-start") {
      ...
      // Conversational follow-up: if the reply asks a question (has "?") or the
      // agent flagged continue_conversation, keep listening without a wake word.
      const wantsFollowup = !!io.continue_conversation || !!(speech && speech.indexOf("?") !== -1);
      const micOk = !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia);
      this._assistPendingContinue = wantsFollowup && this._assistLastInputVoice && micOk && this._assistOpen;
    } else if (t === "tts-end") {
      ...
      if (this._assistPendingContinue) {
        const el = this._assistAudioEl;
        const playing = el && !el.paused && !el.ended;
        if (!playing) setTimeout(() => this._assistMaybeContinue(), 500);
      }
    } else if (t === "error") {
      this._assistStopVoice(false);
      this._assistCleanupRun();
  ...
  async _assistSendText(text) {
    ...
    this._assistLastInputVoice = false;
    this._assistPendingContinue = false;
    this._assistStopVoice(true);
    this._assistCleanupRun();
    ...
    this._assistRenderThread();
    try {
      const runMsg = { type: "assist_pipeline/run", start_stage: "intent", end_stage: "tts", input: { text } };
      if (this._assistPipelineId) runMsg.pipeline = this._assistPipelineId;
      if (this._assistConvId) runMsg.conversation_id = this._assistConvId;
      this._assistUnsub = await this._hass.connection.subscribeMessage(
        (ev) => this._assistPipelineEvent(ev),
  ...
  _assistUnlockAudio() {
    // Play a short silent clip during the user gesture so later spoken replies
    // (which arrive after an async round-trip) are allowed to auto-play.
    try {
      if (!this._assistAudioEl) {
        const el = new Audio();
        el.setAttribute("playsinline", "");
        el.preload = "auto";
        this._assistAudioEl = el;
      }
      const el = this._assistAudioEl;
      el.muted = true;
      el.src = "data:audio/wav;base64,UklGRkQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YSAAAACAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgA==";
      const p = el.play();
      if (p && p.then) p.then(() => { try { el.pause(); el.currentTime = 0; el.muted = false; } catch (_) {} }).catch(() => {});
    } catch (_) {}
  }

  _assistPlayTts(url) {
    try {
      if (!url) { this._assistMaybeContinue(); return; }
      const full = url.startsWith("http") ? url : (this._hass && this._hass.hassUrl ? this._hass.hassUrl(url) : url);
      const el = this._assistAudioEl || (this._assistAudioEl = new Audio());
      try { el.setAttribute("playsinline", ""); } catch (_) {}
      el.muted = false;
      el.volume = 1;
      el.src = full;
      el.onended = () => this._assistMaybeContinue();
      const p = el.play();
      if (p && p.catch) p.catch(() => this._assistMaybeContinue());
    } catch (_) { this._assistMaybeContinue(); }
  }
  ...
  _mount() {
    ...
    wrap.innerHTML = `
      ...
      <main class="sc-main">
        ...
        <div class="sc-studiobar glass" id="sc-studiobar">
          ...
          <div class="sc-studiobar-pa" id="sc-studiobar-pa"></div>
        </div>
      ...
      <div class="sc-modal-root" id="sc-playlist-modal"></div>
      <div class="sc-modal-root" id="sc-music-search-modal"></div>
    ...
    wrap.querySelectorAll(".sc-studiobar-btns [data-act='script']").forEach((el) =>
  ...
  _paintStudiobarPa() {
    const host = this.querySelector("#sc-studiobar-pa");
    if (!host) return;
    // PA master controls only belong on the Media tab.
    if (this._tab !== "media") {
      host.innerHTML = "";
      host.hidden = true;
      return;
    ...
    host.hidden = false;
    const ent = "switch.p_a_speakers";
    const on = this._on(ent);
    const st = (this._state(ent) || "unknown").toUpperCase();
    host.innerHTML = `
      <button class="sc-pa-master ${on ? "on" : ""}" data-act="toggle" data-entity="${ent}" title="Toggle P.A. Speakers">
        <span class="sc-pa-ico">🔊</span>
        <span class="sc-pa-meta">
          <strong>P.A. SPEAKERS</strong>
          <em>${on ? "SYSTEM LIVE" : "SYSTEM OFF"}</em>
        </span>
        <span class="sc-pa-state">${st}</span>
      </button>
      <button class="sc-chip ${on ? "on" : ""}" data-act="turn_on" data-entity="${ent}">PA ON</button>
      <button class="sc-chip off" data-act="turn_off" data-entity="${ent}">PA OFF</button>`;
  ...
  _paint() {
    ...
    if (view && !this._volDragging && !(active && view.contains(active) && ["INPUT", "SELECT"].includes(active.tagName))) {
    ...
    if (this._playlistOpen) this._renderPlaylistModal(false);
  ...
  _nebulaLight() {
    const t = String(this._state("input_text.nebula_target") || "").trim();
    if (t.startsWith("light.")) return t;
    return "light.trillium";
  }

  _nebulaParts(light) {
    const id = light || this._nebulaLight();
    const base = id.replace(/^light\./, "");
    return {
      light: id,
      speed: `number.${base}_speed`,
      intensity: `number.${base}_intensity`,
      palette: `select.${base}_color_palette`,
      name: this._name(id),
    };
  }
  ...
  _nebulaWledLights() {
    ...
    return Object.keys(states)
      ...
      .filter((id) => {
        ...
        return !!states[`number.${base}_speed`] && !!states[`select.${base}_color_palette`];
      })
      .sort((a, b) => this._name(a).localeCompare(this._name(b)));
  }

  _openNebulaDevicePicker() {
    const options = this._nebulaWledLights();
    if (!options.length) {
      this._scToast("No WLED devices found");
      return;
    }
    this._renderPicker({
      title: "NEBULA · DEVICE",
      kind: "nebula-device",
      options,
      current: this._nebulaLight(),
    });
  }

  _openEffectPicker() {
    const light = this._nebulaLight();
  ...
  async _openPalettePicker() {
    const ent = this._nebulaParts().palette;
  ...
  _renderPicker({ title, kind, options, current, gradients }) {
    ...
    const grid = options
      .map((o) => {
        ...
        const label = kind === "nebula-device" ? this._name(o) : o;
        if (kind === "palette") {
          ...
          return `<button type="button" class="sc-pick-item palette ${active}" data-pick="${this._esc(o)}">
            ...
            <span class="sc-pick-name">${this._esc(label)}</span>
          </button>`;
        }
    ...
    search.addEventListener("input", () => {
      ...
      m.querySelectorAll(".sc-pick-item").forEach((el) => {
        ...
        const label = (el.querySelector(".sc-pick-name")?.textContent || "").toLowerCase();
        el.style.display = !q || name.includes(q) || label.includes(q) ? "" : "none";
      });
    });
  ...
  _applyPick(kind, val) {
    const parts = this._nebulaParts();
    if (kind === "effect") this._call("light", "turn_on", { effect: val }, { entity_id: parts.light });
    else if (kind === "palette") this._selectOption(parts.palette, val);
    else if (kind === "nebula-device") {
      this._call("input_text", "set_value", { value: val }, { entity_id: "input_text.nebula_target" });
      this._scToast(`NEBULA → ${this._name(val)}`);
    }
  }
  ...
  _scToast(msg) {
    let t = this.querySelector(".sc-global-toast");
    if (!t) {
      t = document.createElement("div");
      t.className = "sc-global-toast";
      this._root.appendChild(t);
    }
    t.textContent = msg;
    requestAnimationFrame(() => t.classList.add("show"));
    clearTimeout(this._scToastT);
    this._scToastT = setTimeout(() => t && t.classList.remove("show"), 2600);
  }

  _openPlaylistModal() {
    if (this._playlistOpen) return;
    this._playlistOpen = true;
    this._playlistFilter = "";
    this._playlistItems = [];
    this._playlistTrack = null;
    this._playlistLoading = true;
    this._playlistMembershipLoading = false;
    this._lockAppScroll(true);
    this._renderPlaylistModal(true);
    this._playlistPollN = 0;
    this._playlistCreating = false;
    this._playlistCreateName = "";
    this._playlistSyncPoll = false;
    this._loadPlaylistPicker();
  }

  _closePlaylistModal() {
    const m = this.querySelector("#sc-playlist-modal");
    this._playlistOpen = false;
    this._playlistLoading = false;
    this._playlistMembershipLoading = false;
    this._lockAppScroll(false);
    ...
    m.classList.remove("open");
    setTimeout(() => {
      if (!this._playlistOpen) m.innerHTML = "";
    }, 380);
  }

  _renderPlaylistModal(full) {
    const m = this.querySelector("#sc-playlist-modal");
    if (!m) return;
    const track = this._playlistTrack;
    const sub = track
      ? `${track.name || "Track"}${track.artist ? " · " + track.artist : ""}`
      : this._playlistLoading
        ? "Loading current track…"
        : "Pick a playlist";
    const loading = this._playlistLoading || this._playlistMembershipLoading;
    const items = (this._playlistItems || []).slice();
    const q = (this._playlistFilter || "").trim().toLowerCase();
    const filtered = q ? items.filter((p) => (p.name || "").toLowerCase().includes(q)) : items;
    const creating = !!this._playlistCreating;
    const grid = filtered
      .map((p) => {
        const art = p.image ? `background-image:url('${this._esc(p.image)}')` : "";
        const on = p.contains ? " on-playlist" : "";
        const tip = p.contains ? "Tap to remove from this playlist" : "Tap to add to this playlist";
        return `<button type="button" class="sc-pl-item${on}${p.can_add === false ? " readonly" : ""}" data-act="pl-pick" data-uri="${this._esc(p.uri || "")}" data-name="${this._esc(p.name || "")}" title="${tip}" ${p.can_add === false ? "disabled" : ""}>
          <span class="sc-pl-thumb ${art ? "" : "noart"}" style="${art}">${art ? "" : "♫"}</span>
          <span class="sc-pl-name">${this._esc(p.name || "")}</span>
          <span class="sc-pl-mark" aria-hidden="true">${p.contains ? "✓" : "+"}</span>
        </button>`;
      })
      .join("");
    const body = this._playlistError && !items.length
      ? `<div class="sc-pl-empty">${this._esc(this._playlistError)}</div>`
      : loading && !items.length
      ? `<div class="sc-pl-loading"><span class="sc-mx-spin">${this._ic("disc")}</span><span>${this._playlistMembershipLoading ? "Checking playlists…" : "Loading playlists…"}</span></div>`
      : `<div class="sc-pl-grid" id="sc-pl-grid">${grid || `<div class="sc-pl-empty">${q ? "No matches." : "No playlists found."}</div>`}</div>`;

    const createRow = creating
      ? `<div class="sc-pl-create-row">
           <input type="text" class="sc-pick-search sc-pl-create-input" id="sc-pl-create-input" placeholder="New playlist name…" autocomplete="off" autocapitalize="sentences" spellcheck="false" value="${this._esc(this._playlistCreateName || "")}">
           <button type="button" class="sc-pl-create-go" id="sc-pl-create-go">Create</button>
           <button type="button" class="sc-pl-create-cancel" id="sc-pl-create-cancel">Cancel</button>
         </div>`
      : `<button type="button" class="sc-pl-new" id="sc-pl-new">+ New playlist</button>`;

    const footer = `<div class="sc-pl-foot">
        <div class="sc-pl-flash" id="sc-pl-flash" hidden></div>
        <button type="button" class="sc-pl-done" id="sc-pl-done">Done</button>
      </div>`;

    if (full) {
      m.innerHTML = `
      <div class="sc-modal-backdrop" id="sc-pl-bd"></div>
      <div class="sc-modal sc-pl-modal glass">
        <div class="sc-modal-head">
          <div>
            <div class="sc-card-title" style="margin:0">SAVE TO PLAYLIST</div>
            <div class="sc-modal-sub sc-pl-sub">${this._esc(sub)}</div>
          </div>
          <button class="sc-modal-x" id="sc-pl-x">✕</button>
        </div>
        <input type="text" class="sc-pick-search sc-pl-search" id="sc-pl-search" placeholder="Search playlists…" autocomplete="off" autocapitalize="off" spellcheck="false" value="${this._esc(this._playlistFilter || "")}">
        ${createRow}
        ${body}
        ${loading && items.length && this._playlistMembershipLoading ? `<div class="sc-pl-scan">Checking which playlists already have this track…</div>` : ""}
        ${footer}
      </div>`;
      this._bindPlaylistModal(m);
      requestAnimationFrame(() => requestAnimationFrame(() => m.classList.add("open")));
      return;
    }

    const subEl = m.querySelector(".sc-pl-sub");
    if (subEl) subEl.textContent = sub;
    const createHost = m.querySelector(".sc-pl-create-row") || m.querySelector("#sc-pl-new");
    if (createHost) {
      const holder = document.createElement("div");
      holder.innerHTML = createRow.trim();
      createHost.replaceWith(holder.firstElementChild);
    }
    const wrap = m.querySelector(".sc-pl-grid")?.parentElement;
    if (wrap && m.querySelector(".sc-pl-loading")) {
      const holder = document.createElement("div");
      holder.innerHTML = body;
      const oldLoad = m.querySelector(".sc-pl-loading");
      if (oldLoad) oldLoad.replaceWith(holder.firstElementChild);
    } else {
      const gridEl = m.querySelector("#sc-pl-grid");
      if (gridEl) gridEl.outerHTML = body.trim();
      else {
        const loadEl = m.querySelector(".sc-pl-loading");
        if (loadEl) loadEl.outerHTML = body.trim();
        const empty = m.querySelector(".sc-pl-empty");
        if (empty && body.includes("sc-pl-grid")) empty.outerHTML = body.trim();
      }
    }
    let scan = m.querySelector(".sc-pl-scan");
    if (loading && items.length && this._playlistMembershipLoading) {
      if (!scan) {
        scan = document.createElement("div");
        scan.className = "sc-pl-scan";
        scan.textContent = "Checking which playlists already have this track…";
        m.querySelector(".sc-pl-modal")?.insertBefore(scan, m.querySelector(".sc-pl-foot"));
      }
    } else if (scan) scan.remove();
    this._bindPlaylistModal(m);
  }

  _bindPlaylistModal(m) {
    if (!m) return;
    const bd = m.querySelector("#sc-pl-bd");
    const x = m.querySelector("#sc-pl-x");
    const done = m.querySelector("#sc-pl-done");
    if (bd) bd.onclick = () => this._closePlaylistModal();
    if (x) x.onclick = () => this._closePlaylistModal();
    if (done) done.onclick = () => this._closePlaylistModal();
    const search = m.querySelector("#sc-pl-search");
    if (search) {
      search.oninput = () => {
        this._playlistFilter = search.value;
        this._renderPlaylistModal(false);
      };
    }
    const neu = m.querySelector("#sc-pl-new");
    if (neu) neu.onclick = () => {
      this._playlistCreating = true;
      this._playlistCreateName = "";
      this._renderPlaylistModal(false);
      requestAnimationFrame(() => m.querySelector("#sc-pl-create-input")?.focus());
    };
    const cancel = m.querySelector("#sc-pl-create-cancel");
    if (cancel) cancel.onclick = () => {
      this._playlistCreating = false;
      this._playlistCreateName = "";
      this._renderPlaylistModal(false);
    };
    const go = m.querySelector("#sc-pl-create-go");
    const cin = m.querySelector("#sc-pl-create-input");
    if (cin) cin.oninput = () => { this._playlistCreateName = cin.value; };
    if (cin) cin.onkeydown = (ev) => {
      if (ev.key === "Enter") { ev.preventDefault(); this._createPlaylist(); }
    };
    if (go) go.onclick = () => this._createPlaylist();
    m.querySelectorAll("[data-act='pl-pick']").forEach((el) => {
      el.onclick = () => this._toggleTrackPlaylist(el.dataset.uri, el.dataset.name);
    });
  }

  _flashPlaylist(msg, ok = true) {
    const flash = this.querySelector("#sc-pl-flash");
    if (!flash) {
      this._scToast(msg);
      return;
    }
    flash.hidden = false;
    flash.className = `sc-pl-flash show ${ok ? "ok" : "err"}`;
    flash.textContent = msg;
    clearTimeout(this._plFlashT);
    this._plFlashT = setTimeout(() => {
      flash.classList.remove("show");
      setTimeout(() => { flash.hidden = true; }, 220);
    }, 1400);
  }

  _sortPlaylistItems(items) {
    return (items || []).slice().sort((a, b) => {
      const ae = Number(a.last_edited || 0);
      const be = Number(b.last_edited || 0);
      if (ae !== be) return be - ae;
      const ac = a.contains ? 0 : 1;
      const bc = b.contains ? 0 : 1;
  ...
  async _createPlaylist() {
    const name = (this._playlistCreateName || "").trim();
    if (!name || this._playlistSaving) return;
    this._playlistSaving = true;
    this._flashPlaylist(`Creating “${name}”…`, true);
    let resp = null;
    try {
      resp = await this._callResp(
        "launchpad_music",
        "create_playlist",
        { name, add_current: true, entity_id: this._studioPlayer },
        {}
      );
    } catch (e) {
      resp = null;
    }
    this._playlistSaving = false;
    if (!resp || resp.status !== "created" || !resp.playlist) {
      this._flashPlaylist((resp && resp.message) || "Could not create playlist.", false);
      return;
    }
    this._playlistCreating = false;
    this._playlistCreateName = "";
    const pl = resp.playlist;
    this._playlistItems = this._sortPlaylistItems([
      pl,
      ...(this._playlistItems || []).filter((p) => p.uri !== pl.uri),
    ]);
    this._renderPlaylistModal(false);
    this._flashPlaylist(resp.message || `Created ${pl.name}.`, true);
  }

  async _toggleTrackPlaylist(uri, name) {
    if (!uri || this._playlistSaving) return;
    const current = (this._playlistItems || []).find((p) => p.uri === uri);
    const removing = !!(current && current.contains);
    this._playlistSaving = true;
    clearTimeout(this._playlistPollT);
    const btn = this.querySelector(`[data-uri="${CSS.escape(uri)}"]`);
    if (btn) btn.classList.add("busy");
    let resp = null;
    try {
      resp = await this._callResp(
        "launchpad_music",
        removing ? "remove_from_playlist" : "add_to_playlist",
        { playlist_uri: uri, entity_id: this._studioPlayer },
        {}
      );
    } catch (e) {
      resp = null;
    ...
    const ok = resp && (resp.status === "added" || resp.status === "already" || resp.status === "removed" || resp.status === "absent");
    const plName = (resp && resp.playlist && resp.playlist.name) || name || "playlist";
    if (!ok) {
      this._flashPlaylist((resp && resp.message) || `Could not update ${plName}.`, false);
      return;
    }

    const nowOn = resp.status === "added" || resp.status === "already";
    const edited = Number((resp.playlist && resp.playlist.last_edited) || Date.now() / 1000);
    this._playlistItems = this._sortPlaylistItems(
      (this._playlistItems || []).map((p) =>
        p.uri === uri ? { ...p, contains: nowOn, last_edited: edited } : p
      )
    );
    this._renderPlaylistModal(false);
    if (nowOn) this._flashPlaylist(`Saved to ${plName}`, true);
    else this._flashPlaylist(`Removed from ${plName}`, true);
  }

  async _loadPlaylistPicker(opts = {}) {
    ...
    let resp = null;
    try {
      // Membership from server inverted index (instant when warm; cold polls briefly).
      resp = await this._callResp(
        "launchpad_music",
        "playlist_picker",
        { entity_id: this._studioPlayer, check_membership: true },
        {}
      );
    } catch (e) {
      resp = null;
      if (!soft) this._playlistError = String(e?.message || e || "Failed to load playlists");
    }
    if (!this._playlistOpen) return;
    if (!resp) {
      if (!soft) {
        this._playlistLoading = false;
        this._playlistMembershipLoading = false;
        this._playlistError = this._playlistError || "Could not load playlists (service response missing).";
        this._renderPlaylistModal(false);
      }
      return;
    }
    this._playlistError = null;
    this._playlistTrack = (resp && resp.track) || this._playlistTrack;
    const pls = (resp && resp.playlists) || [];
    this._playlistItems = pls.slice().sort((a, b) => {
      const ae = Number(a.last_edited || 0);
      const be = Number(b.last_edited || 0);
      if (ae !== be) return be - ae;
      const ac = a.contains ? 0 : 1;
    ...
    this._playlistLoading = false;
    this._playlistMembershipLoading = !resp.index_warm && !(resp.membership_hits > 0);
    this._renderPlaylistModal(false);
    // Cold index: poll briefly until we have membership hits (green outlines).
    const hasHits = (resp.membership_hits || 0) > 0;
    if (!resp.index_warm && !hasHits && this._playlistOpen) {
      const n = (this._playlistPollN || 0) + 1;
      this._playlistPollN = n;
      if (n <= 12) {
        clearTimeout(this._playlistPollT);
        this._playlistPollT = setTimeout(() => this._loadPlaylistPicker({ soft: true }), 1200);
      } else {
        this._playlistMembershipLoading = false;
        this._renderPlaylistModal(false);
      }
    ...
    // After MA playlist sync kicks, soft-refresh once for new Spotify playlists.
    if (!soft && !this._playlistSyncPoll) {
      this._playlistSyncPoll = true;
      setTimeout(() => {
        this._playlistSyncPoll = false;
        if (this._playlistOpen) this._loadPlaylistPicker({ soft: true });
      }, 3500);
    }
  }
  ...
  _openMusicSearch() {
    if (this._musicSearchOpen) return;
    this._musicSearchOpen = true;
    this._musicSearchQuery = this._musicSearchQuery || "";
    this._musicSearchResults = [];
    this._musicSearchLoading = false;
    this._musicSearchError = null;
    this._lockAppScroll(true);
    this._renderMusicSearch(true);
    requestAnimationFrame(() => {
      const input = this.querySelector("#sc-ms-input");
      if (input) { input.focus(); input.select(); }
      if ((this._musicSearchQuery || "").trim().length >= 2) this._runMusicSearch(true);
    });
  }

  _closeMusicSearch() {
    const m = this.querySelector("#sc-music-search-modal");
    this._musicSearchOpen = false;
    this._musicSearchLoading = false;
    clearTimeout(this._musicSearchDebounce);
    this._lockAppScroll(false);
    if (!m) return;
    m.classList.remove("open");
    setTimeout(() => {
      if (!this._musicSearchOpen) m.innerHTML = "";
    }, 380);
  }
  ...
  _musicSearchPlaceholder() {
    const s = this._musicSearchScope;
    if (s === "artist") return "Search artists…";
    if (s === "playlist") return "Search playlists…";
    if (s === "genre") return "Search genres…";
    return "Search artists, tracks, playlists…";
  }

  _flattenMusicSearch(resp, scope) {
    if (!resp || typeof resp !== "object") return [];
    const order = scope === "artist"
      ? ["artists"]
      : scope === "playlist" || scope === "genre"
        ? ["playlists", "radio"]
        : ["artists", "tracks", "albums", "playlists", "radio", "podcasts", "audiobooks"];
    const labels = {
      artists: { one: "Artist", many: "Artists" },
      tracks: { one: "Track", many: "Tracks" },
      albums: { one: "Album", many: "Albums" },
      playlists: { one: "Playlist", many: "Playlists" },
      radio: { one: "Radio", many: "Radio" },
      podcasts: { one: "Podcast", many: "Podcasts" },
      audiobooks: { one: "Audiobook", many: "Audiobooks" },
    };
    const out = [];
    const seen = new Set();
    for (const key of order) {
      const list = Array.isArray(resp[key]) ? resp[key] : [];
      const lab = labels[key] || { one: key, many: key };
      for (const item of list) {
        ...
        const artists = Array.isArray(item.artists)
          ...
          : "";
        const album = item.album && item.album.name ? item.album.name : "";
        const sub = [artists, album].filter(Boolean).join(" · ");
        out.push({
          uri: item.uri,
          name: item.name || "Untitled",
          media_type: item.media_type || key.replace(/s$/, ""),
          image: item.image || null,
          kind: lab.one,
          section: key,
          sectionLabel: lab.many,
          sub,
        });
      }
    }
  ...
  _musicSearchItemHtml(it, i, { showKind = false } = {}) {
    const art = it.image ? `background-image:url('${this._esc(it.image)}')` : "";
    const sub = showKind ? [it.kind, it.sub].filter(Boolean).join(" · ") : (it.sub || "");
    return `<button type="button" class="sc-ms-item" data-act="ms-play" data-uri="${this._esc(it.uri)}" data-mtype="${this._esc(it.media_type)}" data-name="${this._esc(it.name)}" style="--i:${Math.min(i, 24)}">
      <span class="sc-ms-thumb ${art ? "" : "noart"}" style="${art}">${art ? "" : this._ic("disc")}</span>
      <span class="sc-ms-meta">
        <strong>${this._esc(it.name)}</strong>
        ${sub ? `<em>${this._esc(sub)}</em>` : `<em class="dim">${this._esc(it.kind || "")}</em>`}
      </span>
      <span class="sc-ms-playico" aria-hidden="true">${this._ic("playc")}</span>
    </button>`;
  }

  _musicSearchResultsHtml(items, scope) {
    const list = items || [];
    if (scope !== "all") {
      const grid = list.map((it, i) => this._musicSearchItemHtml(it, i, { showKind: false })).join("");
      return `<div class="sc-ms-grid" id="sc-ms-grid">${grid}</div>`;
    }
    const order = [];
    ...
    const sections = order.map((key) => {
      ...
      const label = (rows[0] && rows[0].sectionLabel) || key;
      const cards = rows.map((it) => this._musicSearchItemHtml(it, i++, { showKind: false })).join("");
      return `<section class="sc-ms-section" data-section="${this._esc(key)}">
        <div class="sc-ms-div" role="presentation"><span>${this._esc(label)}</span><i></i></div>
        <div class="sc-ms-section-list">${cards}</div>
      </section>`;
    }).join("");
    return `<div class="sc-ms-sections all" id="sc-ms-grid">${sections}</div>`;
  }

  _renderMusicSearch(full) {
    const m = this.querySelector("#sc-music-search-modal");
    if (!m) return;
    const scope = this._musicSearchScope || "all";
    const q = this._musicSearchQuery || "";
    const chips = this._musicSearchScopes().map(([id, label]) =>
      `<button type="button" class="sc-ms-chip${scope === id ? " on" : ""}" data-act="ms-scope" data-scope="${id}">${label}</button>`
    ).join("");
    const items = this._musicSearchResults || [];
    const loading = !!this._musicSearchLoading;
    const err = this._musicSearchError;
    const qTrim = q.trim();
    const genreRadio = scope === "genre" && qTrim.length >= 2
      ? `<button type="button" class="sc-ms-radio" data-act="ms-genre-radio" data-q="${this._esc(qTrim)}">
           <span class="sc-ms-radio-ico">${this._ic("disc")}</span>
           <span class="sc-ms-radio-txt"><strong>Start radio</strong><em>${this._esc(qTrim)}</em></span>
           <span class="sc-ms-radio-go">▶</span>
         </button>`
      : "";
    let body;
    if (err && !items.length) {
      body = `<div class="sc-ms-empty">${this._esc(err)}</div>`;
    } else if (loading && !items.length) {
      body = `<div class="sc-ms-loading"><span class="sc-mx-spin">${this._ic("disc")}</span><span>Searching Music Assistant…</span></div>`;
    } else if (!qTrim) {
      body = `<div class="sc-ms-empty">Type to search Music Assistant.<br><small>All returns the best matches across your library and providers.</small></div>`;
    } else if (qTrim.length < 2) {
      body = `<div class="sc-ms-empty">Keep typing…</div>`;
    } else if (!items.length && !loading) {
      body = `<div class="sc-ms-empty">No matches for “${this._esc(qTrim)}”.</div>`;
    } else {
      body = this._musicSearchResultsHtml(items, scope);
    }

    if (full) {
      m.innerHTML = `
      <div class="sc-modal-backdrop" id="sc-ms-bd"></div>
      <div class="sc-modal sc-ms-modal glass">
        <div class="sc-modal-head">
          <div>
            <div class="sc-card-title" style="margin:0">SEARCH MUSIC</div>
            <div class="sc-modal-sub">Music Assistant · ${this._esc(this._name(this._studioPlayer) || "Full Studio")}</div>
          </div>
          <button class="sc-modal-x" id="sc-ms-x" aria-label="Close">✕</button>
        </div>
        <div class="sc-ms-scopes" id="sc-ms-scopes">${chips}</div>
        <div class="sc-ms-inputwrap">
          ${this._ic("search")}
          <input type="search" class="sc-pick-search sc-ms-input" id="sc-ms-input" placeholder="${this._esc(this._musicSearchPlaceholder())}" autocomplete="off" autocapitalize="off" spellcheck="false" enterkeyhint="search" value="${this._esc(q)}">
          <button type="button" class="sc-ms-clear" id="sc-ms-clear" title="Clear" aria-label="Clear" ${q ? "" : "hidden"}>${this._ic("close")}</button>
        </div>
        ${genreRadio}
        <div class="sc-ms-body" id="sc-ms-body">${body}</div>
      </div>`;
      this._bindMusicSearch(m);
      requestAnimationFrame(() => requestAnimationFrame(() => m.classList.add("open")));
      return;
    }

    const scopes = m.querySelector("#sc-ms-scopes");
    if (scopes) scopes.innerHTML = chips;
    const input = m.querySelector("#sc-ms-input");
    if (input && document.activeElement !== input) {
      input.placeholder = this._musicSearchPlaceholder();
      if (input.value !== q) input.value = q;
    }
    const clear = m.querySelector("#sc-ms-clear");
    if (clear) clear.hidden = !q;
    const bodyEl = m.querySelector("#sc-ms-body");
    if (bodyEl) {
      const radioHost = m.querySelector(".sc-ms-radio");
      if (genreRadio) {
        const holder = document.createElement("div");
        holder.innerHTML = genreRadio.trim();
        if (radioHost) radioHost.replaceWith(holder.firstElementChild);
        else bodyEl.before(holder.firstElementChild);
      } else if (radioHost) radioHost.remove();
  ...
  _bindMusicSearch(m) {
    if (!m) return;
    const bd = m.querySelector("#sc-ms-bd");
    const x = m.querySelector("#sc-ms-x");
    if (bd) bd.onclick = () => this._closeMusicSearch();
    if (x) x.onclick = () => this._closeMusicSearch();
    m.querySelectorAll("[data-act='ms-scope']").forEach((el) => {
      el.onclick = () => {
        const next = el.dataset.scope || "all";
        if (next === this._musicSearchScope) return;
        this._musicSearchScope = next;
        this._musicSearchResults = [];
        this._musicSearchError = null;
        this._renderMusicSearch(false);
        const input = m.querySelector("#sc-ms-input");
    ...
    const input = m.querySelector("#sc-ms-input");
    if (input && !input._scMsBound) {
      input._scMsBound = true;
      input.addEventListener("input", () => {
        this._musicSearchQuery = input.value;
        const clear = m.querySelector("#sc-ms-clear");
        if (clear) clear.hidden = !input.value;
        clearTimeout(this._musicSearchDebounce);
        this._musicSearchDebounce = setTimeout(() => this._runMusicSearch(false), 180);
      });
      input.addEventListener("keydown", (ev) => {
        if (ev.key === "Escape") { ev.preventDefault(); this._closeMusicSearch(); }
        if (ev.key === "Enter") {
          ev.preventDefault();
          clearTimeout(this._musicSearchDebounce);
          this._runMusicSearch(true);
        }
      });
    }
    const clearBtn = m.querySelector("#sc-ms-clear");
    if (clearBtn) clearBtn.onclick = () => {
      ...
      this._musicSearchLoading = false;
      this._renderMusicSearch(false);
      m.querySelector("#sc-ms-input")?.focus();
    };
    m.querySelectorAll("[data-act='ms-play']").forEach((el) => {
      el.onclick = () => this._playMusicSearchResult(el.dataset.uri, el.dataset.mtype, el.dataset.name);
    });
    m.querySelectorAll("[data-act='ms-genre-radio']").forEach((el) => {
      el.onclick = () => this._playGenreRadio(el.dataset.q);
    });
  }

  async _runMusicSearch(immediate) {
    if (!this._musicSearchOpen) return;
    const q = (this._musicSearchQuery || "").trim();
    const token = (this._musicSearchToken = (this._musicSearchToken || 0) + 1);
    if (q.length < 2) {
      this._musicSearchResults = [];
      this._musicSearchLoading = false;
      this._musicSearchError = null;
    ...
    this._musicSearchError = null;
    if (immediate || !(this._musicSearchResults || []).length) this._renderMusicSearch(false);
    const scope = this._musicSearchScope || "all";
    const data = {
      config_entry_id: this._massEntryId,
      name: q,
      limit: scope === "all" ? 8 : 16,
    };
    if (scope === "artist") data.media_type = ["artist"];
    else if (scope === "playlist") data.media_type = ["playlist"];
    else if (scope === "genre") data.media_type = ["playlist", "radio"];
    let resp = null;
    try {
      resp = await this._callResp("music_assistant", "search", data, {});
    } catch (e) {
      resp = null;
      if (token === this._musicSearchToken) {
        this._musicSearchError = String(e?.message || e || "Search failed");
      }
    }
    if (!this._musicSearchOpen || token !== this._musicSearchToken) return;
    this._musicSearchLoading = false;
    if (!resp) {
      this._musicSearchResults = [];
      this._musicSearchError = this._musicSearchError || "Music Assistant search unavailable.";
      this._renderMusicSearch(false);
      return;
    }
    this._musicSearchResults = this._flattenMusicSearch(resp, scope);
    this._musicSearchError = null;
    this._renderMusicSearch(false);
  }

  _playMusicSearchResult(uri, mediaType, name) {
    const target = this._studioPlayer || this._mediaTarget();
    if (!target || !uri) return;
    const data = { media_id: uri, enqueue: "replace" };
    if (mediaType) data.media_type = mediaType;
    this._call("music_assistant", "play_media", data, { entity_id: target });
    this._scToast(`▶  ${name || "Playing"}  →  ${this._name(target)}`);
    this._closeMusicSearch();
  }

  _playGenreRadio(q) {
    const target = this._studioPlayer || this._mediaTarget();
    const name = (q || this._musicSearchQuery || "").trim();
    if (!target || !name) return;
    this._call(
      "music_assistant",
      "play_media",
      { media_id: name, radio_mode: true, enqueue: "replace" },
      { entity_id: target }
    );
    this._scToast(`▶  ${name} radio  →  ${this._name(target)}`);
    this._closeMusicSearch();
  }

  _renderNebula() {
    const parts = this._nebulaParts();
    const light = parts.light;
    const st = this._s(light);
    ...
    const status = on
      ? [curEffect || "CUSTOM", curPalette && curPalette !== "unavailable" ? curPalette : ""]
          ...
          .join(" · ")
      : "OFFLINE";
    const subtitle = `${parts.name} · ${status}`;
      ...
      <div class="sc-full glass sc-nebula ${on ? "live" : ""}">
        ...
        <div class="sc-nebula-inner">
          <div class="sc-nebula-head">
            <button type="button" class="sc-nebula-brand" data-act="pick-nebula-device" title="Choose which WLED Nebula controls">
            ...
            <div class="sc-nebula-head-actions">
              <button type="button" class="sc-nebula-voice" data-on="${voiceRing ? 1 : 0}" data-act="toggle" data-entity="input_boolean.voice_trillium_status" title="Assist status colors on Trillium">
                RING ${voiceRing ? 'ON' : 'OFF'}
              </button>
          ...
          <div class="sc-nebula-grid">
            ...
            <div class="sc-nebula-field">
              ...
              <input type="range" class="sc-slider nebula speed" min="0" max="255" step="1" value="${speed}" data-act="number" data-entity="${parts.speed}">
            </div>

            <div class="sc-nebula-field">
              <span class="sc-nebula-lbl">DISTORTION <em class="sc-slider-val">${distortion}</em></span>
              <input type="range" class="sc-slider nebula distort" min="0" max="255" step="1" value="${distortion}" data-act="number" data-entity="${parts.intensity}">
            </div>
    ...
    root.querySelectorAll("[data-act]").forEach((el) => {
      ...
      if (act === "volume") {
        ...
        const send = (force) => {
          ...
          if (!force && lastSent === v) return;
          lastSent = v;
          this._call("media_player", "volume_set", { volume_level: v / 100 }, { entity_id: el.dataset.entity });
        };
        const armDrag = () => {
          this._volDragging = true;
          this._volDragEntity = el.dataset.entity || "";
          this._volDragValue = Number(el.value);
          clearTimeout(this._volDragClearT);
        };
        const clearDrag = () => {
          clearTimeout(this._volDragClearT);
          this._volDragClearT = setTimeout(() => {
            this._volDragging = false;
            this._volDragEntity = null;
          }, 280);
        };
        el.addEventListener("pointerdown", armDrag);
        el.addEventListener("touchstart", armDrag, { passive: true });
        el.addEventListener("pointerup", () => { send(true); clearDrag(); });
        el.addEventListener("touchend", () => { send(true); clearDrag(); });
        el.addEventListener("input", () => {
          armDrag();
          this._volDragValue = Number(el.value);
          if (em) em.textContent = el.value + "%";
          if (!raf) raf = requestAnimationFrame(() => send(false));
        });
        el.addEventListener("change", () => { send(true); clearDrag(); });
        return;
      }
      el.addEventListener("click", (ev) => {
        ev.preventDefault();
        ...
        if (act === "pick-nebula-device") { this._openNebulaDevicePicker(); return; }
        if (act === "open-playlist") { ev.stopPropagation(); this._openPlaylistModal(); return; }
        if (act === "open-music-search") { ev.stopPropagation(); this._openMusicSearch(); return; }
...
.sc-studiobar {
  ...
  gap: 10px; padding: 10px 14px; width: 100%; box-sizing: border-box; min-width: 0;
}
...
.sc-studiobar-pa .sc-pa-master { flex: 1 1 220px; min-width: 0; }
.sc-studiobar-pa .sc-chip { min-width: 0; flex: 0 0 auto; }

.sc-grid { display: grid; grid-template-columns: minmax(0, 1fr); grid-auto-rows: minmax(0, auto); gap: 12px; width: 100%; min-width: 0; }
.sc-col.hero {
  grid-column: 1 / -1;
  min-height: 220px;
  max-height: 340px;
...
.sc-row.reminders { grid-column: 1 / -1; min-width: 0; max-width: 100%; overflow: hidden; }
.sc-full { grid-column: 1 / -1; padding: 20px; min-width: 0; max-width: 100%; overflow-x: hidden; box-sizing: border-box; }
...
/* Reminders block */
.sc-rem-list { display: grid; gap: 10px; min-width: 0; grid-template-columns: 1fr; }
.sc-rem-list:has(.sc-rem:nth-child(2)) { grid-template-columns: repeat(auto-fill, minmax(148px, 1fr)); }
...
.sc-rem { display: flex; align-items: center; gap: 14px; border: 1px solid rgba(216,180,254,0.2); background: rgba(24, 8, 48, 0.42); border-radius: 16px; padding: 13px 14px; transition: 0.2s ease; min-width: 0; max-width: 100%; box-sizing: border-box; }
...
.sc-rem-body { flex: 1; min-width: 0; overflow: hidden; }
...
.sc-rem-when { font-family: Orbitron, sans-serif; font-size: 10px; letter-spacing: 0.14em; color: #c4b5fd; margin-top: 5px; text-transform: uppercase; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem { flex-direction: column; align-items: stretch; justify-content: space-between; gap: 10px; padding: 12px; aspect-ratio: 1 / 1; position: relative; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-tick { position: absolute; top: 12px; left: 12px; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-body { padding-top: 14px; flex: 1; display: flex; flex-direction: column; justify-content: center; min-height: 0; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-msg { white-space: normal; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; text-overflow: ellipsis; font-size: 15px; line-height: 1.25; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-when { white-space: normal; margin-top: 8px; letter-spacing: 0.1em; line-height: 1.35; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-actions { justify-content: flex-end; width: 100%; }
.sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-btn { width: 34px; height: 34px; border-radius: 10px; }
.sc-col, .sc-row { padding: 18px; box-sizing: border-box; min-width: 0; max-width: 100%; }
...
/* mute control replaced by music search */
...
.sc-active-list {
  display: grid; gap: 8px; width: 100%; max-width: 100%;
...
.sc-active-row {
  ...
  border: 1px solid rgba(216,180,254,0.2); background: rgba(24,8,48,0.4); color: #f3e8ff;
...
.sc-active-meta { flex: 1 1 auto; min-width: 0; display: flex; flex-direction: column; overflow: hidden; }
.sc-active-meta strong {
...
.sc-active-row em {
  color: #67e8f9; font-style: normal; font-weight: 700; font-size: 11px; letter-spacing: 0.06em;
  flex: 0 1 auto; max-width: 34%; min-width: 0;
...
/* ---- PA speakers ---- */
.sc-pa-row { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; width: 100%; min-width: 0; }
.sc-pa-master {
...
.sc-pa-meta { flex: 1; min-width: 0; display: flex; flex-direction: column; overflow: hidden; }
.sc-pa-meta strong { letter-spacing: 0.1em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sc-pa-meta em { font-style: normal; color: #a5b4fc; font-size: 12px; letter-spacing: 0.08em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sc-pa-state { font-weight: 700; color: #67e8f9; letter-spacing: 0.1em; flex: 0 0 auto; }
...
.sc-nebula-brand { display: flex; align-items: center; gap: 14px; min-width: 0; border: 0; background: transparent; padding: 0; color: inherit; text-align: left; cursor: pointer; border-radius: 14px; }
.sc-nebula-brand:hover .sc-nebula-orb { transform: scale(1.06); box-shadow: 0 0 22px rgba(168,85,247,0.55); }
.sc-nebula-brand:hover .sc-nebula-sub { color: #e9d5ff; }
.sc-nebula-orb {
...
.sc-nebula-head-actions { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
.sc-nebula-voice { border: 1px solid rgba(56,189,248,0.35); background: rgba(8, 20, 40, 0.55); color: #bae6fd; font-family: Orbitron, sans-serif; font-size: 10px; letter-spacing: 0.14em; padding: 8px 12px; border-radius: 999px; cursor: pointer; transition: 0.15s ease; }
.sc-nebula-voice:hover { transform: translateY(-1px); border-color: rgba(56,189,248,0.7); }
...
@media (max-width: 900px) {
  ...
  .sc-studiobar-pa .sc-pa-master { flex: 1 1 100%; }
  .sc-studiobar-pa .sc-chip { flex: 1; }
  ...
  .sc-rem-list:has(.sc-rem:nth-child(2)) { grid-template-columns: repeat(auto-fill, minmax(136px, 1fr)); gap: 8px; }
  .sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem { padding: 10px; gap: 8px; }
  .sc-rem-list:has(.sc-rem:nth-child(2)) .sc-rem-msg { font-size: 14px; }
...
/* ---- Playlist save modal ---- */
#sc-playlist-modal { z-index: 460; }
#sc-music-search-modal { z-index: 465; }
.sc-ms-modal {
...
@media (min-width: 780px) {
  .sc-ms-modal { width: min(920px, calc(100vw - 40px)); max-height: min(82vh, 820px); }
}
.sc-ms-scopes {
...
.sc-ms-chip {
  ...
  background: rgba(12,6,28,0.55); color: #c4b5fd;
  font: 600 12px/1 Rajdhani, sans-serif; letter-spacing: 0.08em;
...
.sc-ms-chip:hover { border-color: rgba(103,232,249,0.55); color: #e0f2fe; }
.sc-ms-chip.on {
  background: rgba(103,232,249,0.18); border-color: rgba(103,232,249,0.7);
  color: #a5f3fc; box-shadow: 0 0 14px rgba(34,211,238,0.28);
}
.sc-ms-inputwrap {
...
.sc-ms-inputwrap svg { width: 16px; height: 16px; flex: 0 0 auto; }
.sc-ms-input {
...
.sc-ms-clear svg { width: 14px; height: 14px; }
.sc-ms-clear:hover { color: #f9a8d4; background: rgba(244,114,182,0.12); }
.sc-ms-radio {
  display: flex; align-items: center; gap: 12px; width: 100%;
  margin: 0 0 12px; padding: 10px 12px; border-radius: 12px; cursor: pointer;
  border: 1px solid rgba(110,231,183,0.4);
  background: linear-gradient(135deg, rgba(16,185,129,0.16), rgba(56,189,248,0.1));
  color: #ecfdf5; text-align: left;
}
.sc-ms-radio:hover { border-color: rgba(110,231,183,0.75); box-shadow: 0 0 18px rgba(16,185,129,0.25); }
.sc-ms-radio-ico { width: 36px; height: 36px; border-radius: 10px; display: grid; place-items: center;
  background: rgba(12,6,28,0.55); color: #6ee7b7; }
.sc-ms-radio-ico svg { width: 18px; height: 18px; }
.sc-ms-radio-txt { flex: 1; display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.sc-ms-radio-txt strong { font: 700 13px/1.1 Orbitron, sans-serif; letter-spacing: 0.06em; text-transform: uppercase; }
.sc-ms-radio-txt em { font-style: normal; color: #a7f3d0; opacity: 0.9; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sc-ms-radio-go { color: #6ee7b7; font-size: 14px; }
.sc-ms-body { flex: 1; min-height: 160px; overflow: auto; margin: 0 -4px; padding: 0 4px 4px; }
.sc-ms-grid { display: flex; flex-direction: column; gap: 8px; }
.sc-ms-sections {
  display: flex; flex-direction: column; gap: 16px;
}
.sc-ms-section { min-width: 0; display: flex; flex-direction: column; gap: 8px; }
.sc-ms-section-list { display: flex; flex-direction: column; gap: 8px; }
.sc-ms-div {
  display: flex; align-items: center; gap: 10px;
  padding: 0 2px;
}
...
.sc-ms-div i {
  ...
  background: linear-gradient(90deg, rgba(103,232,249,0.55), rgba(167,139,250,0.12), transparent);
}
.sc-ms-item {
  display: flex; align-items: center; gap: 12px; width: 100%;
  padding: 8px 10px; border-radius: 12px; cursor: pointer; text-align: left;
  border: 1px solid rgba(167,139,250,0.22);
  background: rgba(12,6,28,0.45); color: #f3e8ff;
  animation: sc-ms-in .28s ease both; animation-delay: calc(var(--i, 0) * 18ms);
}
.sc-ms-item:hover { border-color: rgba(103,232,249,0.55); background: rgba(34,211,238,0.08); }
.sc-ms-meta em.dim { opacity: 0.55; }
@media (min-width: 780px) {
  .sc-ms-sections.all {
    display: grid;
...
.sc-ms-thumb {
  ...
  background: rgba(12,6,28,0.85) center/cover no-repeat;
  display: grid; place-items: center; color: #a78bfa;
  border: 1px solid rgba(216,180,254,0.2);
}
.sc-ms-thumb svg { width: 18px; height: 18px; }
.sc-ms-meta { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.sc-ms-meta strong {
  font: 600 15px/1.15 Rajdhani, sans-serif; overflow: hidden;
  text-overflow: ellipsis; white-space: nowrap;
}
.sc-ms-meta em {
  font-style: normal; font-size: 12px; color: #a5b4fc; opacity: 0.85;
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.sc-ms-playico { color: #67e8f9; opacity: 0.7; display: grid; place-items: center; }
.sc-ms-playico svg { width: 18px; height: 18px; }
.sc-ms-item:hover .sc-ms-playico { opacity: 1; }
.sc-ms-loading, .sc-ms-empty {
  display: grid; place-items: center; gap: 10px; min-height: 140px;
  color: #c4b5fd; text-align: center; padding: 24px 12px;
}
.sc-ms-empty small { display: block; margin-top: 6px; color: #94a3b8; font-size: 12px; }
.sc-ms-loading { flex-direction: row; }
.sc-transport button.sc-ms-btn { color: #67e8f9; }
.sc-transport button.sc-ms-btn:hover { color: #a5f3fc; filter: drop-shadow(0 0 8px rgba(34,211,238,0.55)); }
@keyframes sc-ms-in {
...
.sc-pl-search { margin-bottom: 12px; }
.sc-pl-grid {
  flex: 1; min-height: 160px; overflow: auto;
...
.sc-pl-item:hover { border-color: rgba(103,232,249,0.55); }
.sc-pl-item.busy { opacity: 0.55; pointer-events: none; }
.sc-pl-item.readonly { opacity: 0.45; cursor: default; }
.sc-pl-thumb {
  position: relative;
  width: 42px; height: 42px; border-radius: 8px; flex: 0 0 auto;
  background: rgba(12,6,28,0.85) center/cover no-repeat;
  display: grid; place-items: center; color: #c084fc; font-size: 16px;
  border: 2px solid transparent;
  box-sizing: border-box;
}
.sc-pl-item.on-playlist .sc-pl-thumb {
  border-color: #34d399;
  box-shadow: 0 0 0 1px rgba(52,211,153,0.35);
}
.sc-pl-thumb.noart { background: rgba(12,6,28,0.85); }
.sc-pl-name {
  flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  font-weight: 600; letter-spacing: 0.03em;
}
.sc-pl-loading, .sc-pl-empty {
  padding: 28px 12px; text-align: center; color: #a5b4fc; letter-spacing: 0.06em;
  display: flex; flex-direction: column; align-items: center; gap: 10px;
}
.sc-pl-scan {
  margin-top: 10px; color: #a5b4fc; font-size: 12px; letter-spacing: 0.08em; text-align: center;
}

.sc-pl-mark {
  flex: 0 0 auto; width: 22px; height: 22px; border-radius: 50%;
  display: grid; place-items: center; font-size: 13px; font-weight: 700;
  color: #a5b4fc; border: 1px solid rgba(165,180,252,0.35);
}
.sc-pl-item.on-playlist .sc-pl-mark {
  color: #6ee7b7; border-color: rgba(52,211,153,0.75);
...
.sc-pl-new {
  ...
  letter-spacing: 0.04em; color: #c4b5fd;
  border: 1px dashed rgba(196,181,253,0.45);
  background: rgba(20,10,40,0.35);
}
.sc-pl-new:hover { border-color: rgba(110,231,183,0.55); color: #6ee7b7; }
.sc-pl-create-row {
  display: flex; gap: 8px; align-items: center; margin-bottom: 10px;
}
.sc-pl-create-input { flex: 1; margin: 0 !important; }
.sc-pl-create-go, .sc-pl-create-cancel, .sc-pl-done {
  cursor: pointer; font: inherit; font-weight: 700; letter-spacing: 0.04em;
  border-radius: 12px; padding: 10px 14px;
  border: 1px solid rgba(216,180,254,0.35);
  background: rgba(20,10,40,0.55); color: #f3e8ff;
}
.sc-pl-create-go, .sc-pl-done {
  border-color: rgba(52,211,153,0.55); color: #6ee7b7;
  background: rgba(6,40,28,0.45);
}
.sc-pl-foot {
  margin-top: 12px; display: flex; flex-direction: column; gap: 10px; align-items: stretch;
}
.sc-pl-done { width: 100%; }
.sc-pl-flash {
...
.sc-pl-flash.show { opacity: 1; transform: translateY(0); }
.sc-pl-flash.ok {
  color: #6ee7b7; border-color: rgba(52,211,153,0.55);
  background: rgba(6,40,28,0.55);
}
.sc-pl-flash.err {
  color: #fda4af; border-color: rgba(251,113,133,0.55);
  background: rgba(60,10,24,0.55);
}
.sc-transport button.sc-pl-save { color: #c4b5fd; }
.sc-transport button.sc-pl-save:hover { color: #6ee7b7; }
.sc-pl-confirm {
  flex: 1; min-height: 220px;
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  gap: 10px; text-align: center; padding: 28px 16px;
  animation: sc-pl-confirm-in 0.22s ease-out;
}
.sc-pl-confirm-mark {
...
.sc-pl-confirm.ok .sc-pl-confirm-mark {
  color: #6ee7b7;
  border-color: rgba(52,211,153,0.7);
  background: rgba(6,40,28,0.55);
  box-shadow: 0 0 28px rgba(52,211,153,0.35);
}
.sc-pl-confirm.err .sc-pl-confirm-mark {
  color: #fda4af;
...
.sc-pl-confirm-detail {
  max-width: 28em; color: #c4b5fd; letter-spacing: 0.03em;
  line-height: 1.35; font-size: 0.95rem;
}
@keyframes sc-pl-confirm-in {
...
.sc-global-toast {
  ...
  padding: 12px 18px; border-radius: 14px; max-width: min(90vw, 420px);
  background: rgba(16,8,36,0.92); color: #f3e8ff;
  border: 1px solid rgba(52,211,153,0.45);
  box-shadow: 0 0 24px rgba(52,211,153,0.25);
  font-weight: 600; letter-spacing: 0.04em; text-align: center;
  transition: opacity 0.2s ease, transform 0.2s ease;
}
.sc-global-toast.show { opacity: 1; transform: translateX(-50%) translateY(0); }
`;
