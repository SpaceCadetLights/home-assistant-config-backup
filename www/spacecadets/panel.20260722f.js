...
class SpaceCadetsPanel extends HTMLElement {
  ...
  async _addTrackToPlaylist(uri, name) {
    ...
    const resp = await this._callResp(
      "launchpad_music",
      "add_to_playlist",
    ...
    const msg = (resp && resp.message) || (resp && resp.status === "added" ? `Added to ${name}.` : `Already on ${name}.`);
    this._scToast(msg);
    if (resp && (resp.status === "added" || resp.status === "already")) {
    ...
    setTimeout(() => this._closePlaylistModal(), resp && resp.status === "already" ? 900 : 1200);
  }
